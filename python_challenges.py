...
# Print a list of all possible coordinates given by (i, j, k) on a 3D grid where the sum of i + j + k  is not equal to n. Please use list comprehensions 
...
if __name__ == '__main__':
    ...
    z = int(input())
    n = int(input())
new_array = [[i, j, k] for i in range(x + 1) for j in range(y + 1) for k in range(z + 1) if i + j + k != n]
print(new_array)